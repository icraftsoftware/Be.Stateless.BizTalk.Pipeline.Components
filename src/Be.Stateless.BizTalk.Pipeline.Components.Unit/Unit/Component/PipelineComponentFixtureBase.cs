...
namespace Be.Stateless.BizTalk.Unit.Component
{
	...
	{
		/// <summary>
		/// All the properties of <see cref="PipelineComponent"/> <typeparamref name="T"/> that are to be read and written to an
		/// <see cref="IPropertyBag"/>.
		/// </summary>
		/// <remarks>
		/// Any property that is <c>public</c> and optionally qualified by a <see
		/// cref="BrowsableAttribute">[Browsable(true)]</see> attribute is considered to be a configurable <see
		...
		protected virtual IEnumerable<PropertyInfo> ConfigurableProperties
		{
			get
			{
				// all public properties that are either [Browsable(true)] qualified or not qualified at all (.All() is true if the
				// collection of BrowsableAttribute is empty)
		...
		protected void VerifyExecuteReturnsImmediatelyWhenMessageIsNull()
		{
			...
			if (resultMessage != null)
				throw new NotSupportedException($"{nameof(PipelineComponent)} {sut.GetType().Name} did not return a null message although it was given a null one.");
		...
		protected void VerifyExecuteThrowsWhenPipelineContextIsNull()
		{
			...
			try
			{
				...
				throw new NotSupportedException($"{nameof(PipelineComponent)} {sut.GetType().Name} did not throw when given a null pipelineContext.");
			}
			catch (ArgumentNullException exception)
			{
				if (exception.ParamName != "pipelineContext")
					throw new NotSupportedException($"{nameof(PipelineComponent)} {sut.GetType().Name} did not throw when given a null pipelineContext.", exception);
			}
		}
